...
#sort the null ethnicity in a group (the ones that done't have a categroy in ONSGroup)
#Gender here is only peron, if we are to include males and females we need to add it to few of the joining
...
con2<-dbConnect(odbc::odbc(), .connection_string = "Driver={SQL Server};server=MLCSU-BI-SQL;database=Working", timeout = 10)
...
#unique(indicator_data$EthnicCategoryCode)
...
#JA:Denominator will not be zero yearly so I just added the DataQyalityID column in the group by
...
s_BSol_by_ethnicity <- pop_ward %>%
  ...
  #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
  group_by(Fiscal_Year,ONSGroup,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>% 
  mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
...
s_BSol_by_IMD <- pop_ward %>%
  ...
  #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
  group_by(Fiscal_Year,quantile,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>% 
  mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
...
#JA: add the DataQualityID from the new table after joining with the rate table
indicator_rate_BSol_by_IMD<-indicator_rate_BSol_by_IMD %>% 
...
s_BSol_by_ethnicityXIMD <- pop_ward %>%
  left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                             "NHSCode" = "EthnicCategoryCode",
                             "Fiscal_Year" = "Fiscal_Year",
                             "Age_B_18_categories_Code" ="AgeGroup_5_Code"
...
#JA: add the DataQualityID from the new table after joining with the rate table
indicator_rate_BSol_by_ethnicityXIMD<-indicator_rate_BSol_by_ethnicityXIMD %>% 
                                  mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
...
s_LA_by_ethnicity <- pop_ward %>%
  ...
  #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
  group_by(Fiscal_Year,LAD22CD.y,ONSGroup,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>% 
  mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
...
#JA: add the DataQualityID from the new table after joining with the rate table
indicator_rate_LA_by_ethnicity<-indicator_rate_LA_by_ethnicity %>%  left_join(s_LA_by_ethnicity, by = c("Ethnicity" = "ONSGroup",
                                                                                                        "AggID"="LAD22CD.y",
...
#IMD indicator rate for local authority
  ...
  s_LA_by_IMD <- pop_ward %>%
    ...
    #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
    group_by(Fiscal_Year,LAD22CD.y,quantile,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>% 
    mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
  ...
  indicator_rate_LA_by_IMD<-indicator_rate_LA_by_IMD %>% 
    mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
  ...
  #ethnicity by IMD indicator rate for LA #JA added  !!!!!!Will result in small numbers but can be better in the 3-5 years rates
  indicator_rate_LA_by_ethnicityXIMD <- pop_ward %>%
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code"
    )) %>%left_join(lsoa_LAD, by=c("Electoral_wards_and_divisions_Code"="WD22CD")) %>% 
    #filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>%  #JA:remove
    group_by(LAD22CD.y,quantile, ONSGroup, Fiscal_Year,Age_B_18_categories_Code,Age_B_18_categories) %>%
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>%
    mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
           Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
    ...
    #filter(!is.na(quantile) & Denominator >0) %>% JA: remove
    group_by(AggType, AggID, Gender, IMD, Ethnicity, Fiscal_Year) %>%
    left_join(Std_pop, by=c("Age_B_18_categories_Code"="AgeBand_Code")) %>%
    phe_dsr(x=Numerator,
            n=Denominator,
            stdpop= Population,
            stdpoptype = "field",
  ...
  s_LA_by_ethnicityXIMD <- pop_ward %>%
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code"
  ...
  #JA: add the DataQualityID from the new table after joining with the rate table
  indicator_rate_LA_by_ethnicityXIMD<-indicator_rate_LA_by_ethnicityXIMD %>% 
    mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
    left_join(s_LA_by_ethnicityXIMD, by = c("q" = "quantile",
                                              "Ethnicity"="ONSGroup",
                                            "AggID"="LAD22CD.y",
                                              "Fiscal_Year" = "Fiscal_Year")) %>% 
...
##JA: to flag the denominator=0 after the grouping but without the age split, we produce a new table (grouped without age grouping)
#with the correct flag
s_Locality_by_ethnicity <- pop_ward %>%
  left_join(indicator, by = c("Electoral_wards_and_divisions_Code" = "WD22CD",
                              "NHSCode" = "EthnicCategoryCode",
                              "Fiscal_Year" = "Fiscal_Year",
                              "Age_B_18_categories_Code" ="AgeGroup_5_Code"
  ...
  #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
  group_by(Fiscal_Year,Locality.y,ONSGroup,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>% 
  mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
...
#JA: add the DataQualityID from the new table after joining with the rate table
indicator_rate_Locality_by_ethnicity<-indicator_rate_Locality_by_ethnicity %>%  left_join(s_Locality_by_ethnicity, by = c("Ethnicity" = "ONSGroup",
                                                                                                        "AggID"="Locality.y",
...
#IMD indicator rate for Locality
  ...
  s_Locality_by_IMD <- pop_ward %>%
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code"
    ))%>%left_join(localitiesWard, by=c("Electoral_wards_and_divisions_Code"="WD22CD")) %>%
    #filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>%  #JA:remove
  ...
  #JA: add the DataQualityID from the new table after joining with the rate table
  indicator_rate_Locality_by_IMD<-indicator_rate_Locality_by_IMD %>% 
    mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
  ...
  #ethnicity by IMD indicator rate for Locality #JA added  !!!!!!Will result in small numbers but can be better in the 3-5 years rates
  indicator_rate_Locality_by_ethnicityXIMD <- pop_ward %>%
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code"
    ...
    #filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>%  #JA:remove
    group_by(Locality.y,quantile, ONSGroup, Fiscal_Year,Age_B_18_categories_Code,Age_B_18_categories) %>%
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>%
    mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
           Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
  ...
  #JA: add the DataQualityID from the new table after joining with the rate table
  indicator_rate_Locality_by_ethnicityXIMD<-indicator_rate_Locality_by_ethnicityXIMD %>% 
    mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
    left_join(s_Locality_by_ethnicityXIMD, by = c("q" = "quantile",
                                            "Ethnicity"="ONSGroup",
                                            "AggID"="Locality.y",
                                            "Fiscal_Year" = "Fiscal_Year")) %>% 
    select(-"total_count",-"total_pop",-"q")
...
indicator_rate_ward <- pop_ward %>%
 ...
 # filter(Denominator > 0 ) %>%
  ...
  s_Ward <- pop_ward %>%
    ...
    #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
    group_by(Fiscal_Year,WD22NM.y,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>% 
    mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
  ...
  indicator_rate_ward<-indicator_rate_ward %>%  left_join(s_Ward, by = c( "AggID"="WD22NM.y",
   ...
   #rates for wards by ethnicity  !!!!!!Will result in small numbers but can be better in the 3-5 years rates
  indicator_rate_ward_by_ethnicity <- pop_ward %>%    
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code" #JA
    ...
    #JA:better join locality to pop_ward from the begnning!!
    group_by(Fiscal_Year, WD22NM.y, ONSGroup,Age_B_18_categories_Code,Age_B_18_categories) %>%
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>%
    mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
           Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
  ...
  s_Ward_by_ethnicity <- pop_ward %>%
    ...
    #  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
    group_by(Fiscal_Year,WD22NM.y,ONSGroup,DataQualityID) %>% #,Age_B_18_categories_Code,Age_B_18_categories,DataQualityID) %>% ##, NHSCode
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>% 
    mutate(DataQualityID = ifelse(Denominator==0,5,DataQualityID))
  ...
  #JA: add the DataQualityID from the new table after joining with the rate table
  indicator_rate_ward_by_ethnicity<-indicator_rate_ward_by_ethnicity %>%  left_join(s_Ward_by_ethnicity, by = c("Ethnicity" = "ONSGroup",
                                                                                                                "AggID"="WD22NM.y",
  ...
  #IMD indicator rate for ward  !!!!!!Will result in small numbers but can be better in the 3-5 years rates
  indicator_rate_ward_by_IMD <- pop_ward %>%
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code" #JA
    ...
    #JA:better join locality to pop_ward from the begnning!!
    group_by(Fiscal_Year, WD22NM.y, quantile,Age_B_18_categories_Code,Age_B_18_categories) %>%
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>%
    mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
           Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
  ...
  #JA: add the DataQualityID from the new table after joining with the rate table
  indicator_rate_ward_by_IMD<-indicator_rate_ward_by_IMD %>% 
    mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
  ...
  #ethnicity by IMD indicator rate for ward  #JA added  !!!!!!Will result in small numbers but can be better in the 3-5 years rates
  indicator_rate_ward_by_ethnicityXIMD <- pop_ward %>%
    left_join(indicator, by= c("Electoral_wards_and_divisions_Code" = "WD22CD",
                               "NHSCode" = "EthnicCategoryCode",
                               "Fiscal_Year" = "Fiscal_Year",
                               "Age_B_18_categories_Code" ="AgeGroup_5_Code"
    ...
    #filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>%  #JA:remove
    group_by(WD22NM.y,quantile, ONSGroup, Fiscal_Year,Age_B_18_categories_Code,Age_B_18_categories) %>%
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>%
    mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
           Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
  ...
  #JA: add the DataQualityID from the new table after joining with the rate table
  indicator_rate_ward_by_ethnicityXIMD<-indicator_rate_ward_by_ethnicityXIMD %>% 
    mutate(q=as.numeric(gsub('Q', '', IMD))) %>%
    left_join(s_Ward_by_ethnicityXIMD, by = c("q" = "quantile",
                                                  "Ethnicity"="ONSGroup",
                                                  "AggID"="WD22NM.y",
                                                  "Fiscal_Year" = "Fiscal_Year")) %>% 
