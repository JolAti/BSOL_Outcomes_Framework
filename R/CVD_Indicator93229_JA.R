...
l#soa_ward_lad_map <- read.csv("C:/Users/Richard.Wilson/Downloads/Lower_Layer_Super_Output_Area_(2021)_to_Ward_(2022)_to_LAD_(2022)_Lookup_in_England_and_Wales_v3.csv", header=TRUE, check.names=FALSE)
...
indicator <- indicator_data %>%
  ...
  group_by(EthnicCategoryCode, Fiscal_Year, LAD22CD,
           WD22CD, WD22NM, Locality,AgeGroup_5,AgeGroup_5_Code) %>%  #JA
  summarise(Numerator = sum(Numerator, na.rm = TRUE)) %>%
  select(Numerator, EthnicCategoryCode, Fiscal_Year, LAD22CD,
...
pop_ward<- popfile_ward %>%
  group_by(Electoral_wards_and_divisions_Code,Electoral_wards_and_divisions,  Ethnic_group_20_categories_Code,
           NHSCode, NHSCodeDefinition, quantile ,Age_B_18_categories_Code,Age_B_18_categories) %>%
...
#  filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% #JA:removed
  group_by(Fiscal_Year, NHSCode,Age_B_18_categories_Code,Age_B_18_categories) %>%
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>%
...
group_by(AggID, Gender, IMD, Ethnicity, Fiscal_Year) %>%
  left_join(Std_pop, by=c("Age_B_18_categories_Code"="AgeBand_Code")) %>% #JA: Added
  phe_dsr(x=Numerator,
          n=Denominator,
          stdpop= Population,
...
indicator_rate_BSol_by_ethnicityXIMD <- pop_ward %>%
  ...
  group_by(quantile, NHSCode, Fiscal_Year,Age_B_18_categories_Code,Age_B_18_categories) %>%
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>%
...
indicator_rate_LA_by_ethnicity <- pop_ward %>%
  ...
  #filter((LAD22CD == 'E08000025' | LAD22CD == 'E08000029') ) %>% ##JA:remove
  group_by(Fiscal_Year,LAD22CD.y, NHSCode,Age_B_18_categories_Code,Age_B_18_categories) %>% #JA added age
  summarise(Numerator = sum(Numerator, na.rm = TRUE),
            Denominator = sum(Denominator)) %>%
  mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
         Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
...
indicator_rate_Locality_by_ethnicity <- pop_ward %>%
  ...
  #JA:better join locality to pop_ward from the begnning!!
    group_by(Fiscal_Year, Locality.y, NHSCode,Age_B_18_categories_Code,Age_B_18_categories) %>%
    summarise(Numerator = sum(Numerator, na.rm = TRUE),
              Denominator = sum(Denominator)) %>%
  mutate(Numerator = ifelse((is.na(Numerator)| Denominator==0), 0, Numerator), ##JA:see above
         Denominator=ifelse(Denominator==0,1,Denominator), ###JA: see above
